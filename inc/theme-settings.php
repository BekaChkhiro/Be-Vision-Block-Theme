...
class BeVision_Theme_Settings {
    ...
    public function register_settings() {
        register_setting('bevision_settings', 'bevision_custom_font', array(
            'type' => 'string',
    ...
    public function handle_font_upload($option) {
        if (!empty($_FILES['custom_font']['name'])) {
            ...
            if (!in_array($file_ext, $allowed_types)) {
                ...
                return get_option('bevision_custom_font');
            }
            ...
            if ($movefile && !isset($movefile['error'])) {
                ...
                $old_font = get_option('bevision_custom_font');
    ...
    public function create_settings_page() {
        ...
        ?>
        <div class="wrap bevision-settings-wrap">
            <h1><?php echo esc_html(get_admin_page_title()); ?></h1>
            
            <?php settings_errors(); ?>
            
            <div class="bevision-font-preview">
                <?php
                $current_font = get_option('bevision_custom_font');
            ...
            <form method="post" action="options.php" enctype="multipart/form-data">
                <?php
                settings_fields('bevision_settings');
                do_settings_sections('bevision-settings');
                ?>
    ...
    public function enqueue_admin_scripts($hook) {
        if ($hook != 'toplevel_page_bevision-settings') {
            return;
        }
        wp_enqueue_style('bevision-admin-styles', get_template_directory_uri() . '/assets/css/admin.css');
    ...
    public function enqueue_custom_font() {
        $custom_font = get_option('bevision_custom_font');
        if ($custom_font && isset($custom_font['url'])) {
            $font_url = $custom_font['url'];
            $font_family = 'BeVisionCustomFont';
            
            wp_enqueue_style('bevision-custom-font', get_template_directory_uri() . '/assets/css/custom-font.css');
            ...
            ";
            wp_add_inline_style('bevision-custom-font', $custom_css);
        }
    }
