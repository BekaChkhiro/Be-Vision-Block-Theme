...
function bevision_render_products_cpt_block($attributes, $content) {
    ...
    $output = '<section class="section-wrapper" style="max-width: 1250px; margin: 0 auto; padding: 70px 0px 120px;  box-sizing: border-box; color: #333; position: relative; overflow: hidden; margin-bottom: 60px;">';
    $output .= '<style>
    ...
    $output .= '<h2 class="products-section-title" style="color: #6653C6; text-align: center; font-size: 18px; font-style: normal; font-weight: 600; line-height: normal; margin: 0px 0px 5px;">' . esc_html($section_title) . '</h2>';
    $output .= '<h3 class="products-section-subtitle" style="color: var(--Dark-Blue, #221A4C); text-align: center; font-size: 24px; font-style: normal; font-weight: 600; line-height: normal; margin: 0px 0px 30px;">' . esc_html($section_subtitle) . '</h3>';
    
    // Build tabs navigation
    $output .= '<ul class="products-tabs-nav" style="display: flex; justify-content: center; gap: 10px; list-style: none; padding: 0; margin-bottom: 60px;">';
    foreach ($products as $product) {
        ...
        $output .= '<li class="products-tab-item' . $active_class . '" data-tab="' . esc_attr($product['id']) . '" style="font-size: 20px; cursor: pointer; transition: all 0.3s ease; padding: 12px 24px; font-weight: 700; ' . $active_style . '">';
    ...
    foreach ($products as $product) {
        ...
        $output .= '<div class="products-grid" style="display: grid; grid-template-columns: 55% 45%; gap: 48px; align-items: top;">';
        
        // Left column with product info
        $output .= '<div class="products-content-left" style="padding: 0;">';
        $output .= '<p class="products-subtitle" style="color: #7B61FF; font-size: 18px; font-weight: 600; margin: 0px 0px 5px;">' . esc_html($product['subtitle']) . '</p>';
        $output .= '<h4 class="products-title" style="color: #333; font-size: 24px; font-weight: 600; margin: 0px 0px 40px;">' . esc_html($product['title']) . '</h4>';
        
        // Features list
        if (!empty($product['features'])) {
            $output .= '<ul class="products-feature-list" style="list-style: none; padding: 0; margin-bottom: 32px;">';
            foreach ($product['features'] as $feature) {
                $output .= '<li class="products-feature-item" style="position: relative; padding-left: 28px; margin-bottom: 20px; color: #333; font-size: 18px; font-weight: 400;">';
                $output .= '<span style="position: absolute; left: 0; top: 2px; color: #4CD964; font-size: 18px;">✓</span>';
                $output .= esc_html($feature);
                $output .= '</li>';
            }
            $output .= '</ul>';
        }
        ...
        if ($product['id'] !== 'default-product') {
            ...
            if (!empty($product['link'])) {
                $output .= '<a href="' . esc_url($product['link']) . '" class="products-button products-button-primary" style="background-color: #6653C6; color: #fff; padding: 15px 40px; font-size: 16px; font-style: normal; font-weight: 400; line-height: normal; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; text-decoration: none; display: flex; align-items: center; justify-content: center;">' . esc_html($primary_button_text) . '</a>';
            }
            
            // Book a Call button (opens popup)
            $output .= '<button class="products-button products-button-secondary open-lead-popup" style="background-color: #2FCA02; color: #fff; padding: 15px 40px; font-size: 16px; font-style: normal; font-weight: 700; line-height: normal; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center;">' . esc_html($secondary_button_text) . '</button>';
